...
esplay_scale_option scale_opt = SCALE_NONE;
...
int player_gnuboy_main(const char * rompath) {
    ...
    // Load ROM
    gnuboy_loader_init(rompath);

    echo_DMA("before create disp buff") ;
    ...
    dn(AUDIO_BUFFER_SIZE)
    echo_DMA("before create audio buff 0") ;
    ...
    memset(&lastJoysticState, 0, sizeof(lastJoysticState));
    ...
    while (true)
    {
        ...
        if (actualFrameCount == 60) {
            float seconds = totalElapsedTime / (CPU_FREQ_MHZ * 1000000.0f); // 240000000.0f; // (240Mhz)
