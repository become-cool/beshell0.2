...
const ADDR = 0x68
class Poser extends EventEmitter {
    constructor(bus) {
        super()
        this.bus = bus || 0
        this._dmpdata = null
    ...
    async readDMP(eurer) {
        ...
        for(let i=0;i<10;i++) {
            if(fifocnt>=42) {
                this._dmpdata = beapi.i2c.read(0, ADDR, 0x74, 16)
                ...
                return eurer? this.toEuler(): this.q
            }
            await delay(1)
            fifocnt = this.fifoCount()
        }
    ...
    setup(bus, sda, scl, freq) {
        ...
        beapi.i2c.setup(bus, sda, scl, freq)
        return this
    }
    async begin(dmp) {
