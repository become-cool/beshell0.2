...
uint16_t ff_convert (    /* Converted code, 0 means conversion error */
    ...
    uint16_t    dir        /* 0: Unicode to OEM code, 1: OEM code to Unicode */
)
{
    const uint16_t *p;
    uint16_t c;
...
*    形    参: utf8 输入字符串
*              gbk  输出字符串
*              gbk_size 字符串size   ,一个汉字2字节，size=汉字个数*2+2
*    返 回 值: 字符串
*********************************************************************************************************
*/
// char *StrUTF8ToGBK(char *utf8, char *gbk)
// {
...
//     while (*ptr != 0)
//     {
//         code1 = *ptr; /* 读取字符串数据， 该数据可能是ascii代码，也可能汉字代码的高字节 */
//         if (code1 < 0x80)
//         {
//           //  if (len + 1 < gbk_size)
//             {
...
//                         break;
//                     }
//                     unicode = ((uint32_t)(code1 & 0x1F) << 6) + (code2 & 0x3F);
//                 }
...
//                         break;
//                     }
//                     unicode = ((uint32_t)(code1 & 0x0F) << 12) + ((uint32_t)(code2 & 0x3F) << 6) + (code3 & 0x3F);
//                    // print(0," unicode1=%x  ", unicode);
//                 }
...
//                     break;
//                 }
//                 gb = ff_convert(unicode, 0);    /* Unicode -> OEM */
//               //  print(0,"gb %x  ",gb);
//                 code1 = gb >> 8;
//                 code2 = gb;
...
uint16_t GBKToStrUTF8( char *gbkin,char *utf8out, uint16_t outlen) {
    ...
    uint16_t lastAsciiPos = outlen - 2 ;
    uint16_t lastUtfPos = outlen - 4 ;
    ...
    while (gbkin[i] != 0) {
        if(gbkin[i] <0x80) {
            ...
            gb |=gbkin[i];
            // printf("gb=%x ",gb);
            unicode=  ff_convert(gb, 1);
            // printf("gb to unicode=%x ",unicode);
            // printf("UTF8=%x %x %x\n", ((unicode >> 12) & 0x0F) | 0xE0,((unicode >>  6) & 0x3F) | 0x80,(unicode& 0x3F) | 0x80 );

            utf8out[len++]= ((unicode >> 12) & 0x0F) | 0xE0;
            utf8out[len++]= ((unicode >>  6) & 0x3F) | 0x80;
            utf8out[len++]= (unicode& 0x3F) | 0x80;
        }
      	i++;
    }
    return len;
