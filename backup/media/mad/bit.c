			   ...
			   unsigned short init)
{
  ...
  for (crc = init; len >= 32; len -= 32) {
    ...
    crc = (crc << 8) ^ unalShort((int)&crc_table[((crc >> 8) ^ (data >> 24)) & 0xff]);
    crc = (crc << 8) ^ unalShort((int)&crc_table[((crc >> 8) ^ (data >> 16)) & 0xff]);
    crc = (crc << 8) ^ unalShort((int)&crc_table[((crc >> 8) ^ (data >>  8)) & 0xff]);
    crc = (crc << 8) ^ unalShort((int)&crc_table[((crc >> 8) ^ (data >>  0)) & 0xff]);
  }

  switch (len / 8) {
  case 3: crc = (crc << 8) ^
	    unalShort((int)&crc_table[((crc >> 8) ^ mad_bit_read(&bitptr, 8)) & 0xff]);
  case 2: crc = (crc << 8) ^
	    unalShort((int)&crc_table[((crc >> 8) ^ mad_bit_read(&bitptr, 8)) & 0xff]);
  case 1: crc = (crc << 8) ^
	    unalShort((int)&crc_table[((crc >> 8) ^ mad_bit_read(&bitptr, 8)) & 0xff]);
