 ...
 * 
 */
/* globals require, FS, Buffer, console.log, ESP32, _sockets*/
...
function sendFile(fileName, response) {
   ...
   while(1) {
   	...
   	// console.log("Size read: " + sizeRead);
   	if (sizeRead <= 0) {
   		break;
   	}
...
function requestHandler(request, response) {
   console.log("WebServer: We have received a new HTTP client request!");
   var postData = "";
   
   request.on("data", function(data) {
      console.log("HTTP Request on(data) passed: \"" + data + "\"");
      postData += data;
   }); // on("data")
   
   request.on("end", function() {
      console.log("HTTP Request on(end):");
      console.log(" - method: " + request.method);
      console.log(" - path: " + request.path);
      console.log(" - headers: " + JSON.stringify(request.headers));

      console.log("URL: " + JSON.stringify(URL.parse(request.path)));
      ...
      if (pathParts[0] == "run" && request.method == "POST") {
      	...
      	console.log("We are about to run: " + postData);
      	try {
      		eval(postData);
      	} catch(e) {
         ...
         response.writeHead(200);
      	...
      	if (pathParts.length == 1 && request.method == "GET") {
         	...
         	response.write(JSON.stringify(filesArray));
      		...
      		if (request.method == "GET") {
            	console.log("Load the file called " + fileName);
	      ...
	      try {
	         ...
	         response.writeHead(200);
	      	...
	      	console.log("We got an exception: " + e);
