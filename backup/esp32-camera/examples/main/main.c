...
void app_main()
{
    ...
    ESP_LOGI(TAG, "free: %d @start", esp_get_free_heap_size());
    
    init_wifi() ;
    ESP_LOGI(TAG, "free: %d after init_wifi()", esp_get_free_heap_size());

    if(ESP_OK != init_camera()) {

        while (1) {
            ESP_LOGI(TAG, "init_camera() failed");
            vTaskDelay(5000 / portTICK_RATE_MS);
        }

        return;
    }
    ESP_LOGI(TAG, "free: %d after init_camera()", esp_get_free_heap_size());
