...
void spi_master_init(TFT_t * dev, int16_t GPIO_MOSI, int16_t GPIO_SCLK, int16_t GPIO_CS, int16_t GPIO_DC, int16_t GPIO_RESET, int16_t GPIO_BL)
{
	...
	spi_bus_config_t buscfg = {
		...
		.quadwp_io_num = -1,
		.quadhd_io_num = -1,
		.max_transfer_sz = 40 * SCREEN_WIDTH * 2 * 8
	};
	...
	// 时钟
	int frequency = SPI_MASTER_FREQ_26M;
	if(GPIO_MOSI==13 && GPIO_SCLK==14 && GPIO_CS==15) {
		// frequency = APB_CLK_FREQ/80 ;  // 1m
		// frequency = SPI_MASTER_FREQ_80M ;
...
bool spi_master_write_byte(spi_device_handle_t SPIHandle, const uint8_t* Data, size_t DataLength)
{
	...
	if ( DataLength > 0 ) {
		...
		SPITransaction.flags = SPITransaction.flags & (~SPI_TRANS_USE_RXDATA) ;
#if 1
...
void lcdInitST7789(TFT_t * dev, int width, int height, int offsetx, int offsety)
{
	printf("SCREEN_DRIVER=%d\n", SCREEN_DRIVER) ;
	printf("offset=%d,%d; width/height=%d/%d\n",offsetx,offsety,width,height) ;

#if SCREEN_DRIVER == SCREEN_DRIVER_ST7789
	printf("lcdInitST7789() driver: st7789\n") ;
	...
	spi_master_write_command(dev, 0x11);	// 唤醒
	delayMS(120);
	...
	// 	spi_master_write_data_byte(dev, 0x20|0x80);	
	// 	// 屏幕开始位置偏移80像素
	// 	spi_master_write_command(dev, 0x37);
	// 	spi_master_write_data_byte(dev, 0);	
	// 	spi_master_write_data_byte(dev, 0x50);	
	// }
	// else {
		spi_master_write_command(dev, 0x36);	
		spi_master_write_data_byte(dev, 0);	
	...
	spi_master_write_command(dev, 0x3A);	// 颜色模式： RGB565 (16Bit)
	spi_master_write_data_byte(dev, 0x05);

	spi_master_write_command(dev, 0xC6);	// 帧率: 60hz
	...
	spi_master_write_command(dev, 0x29);	// 打开屏幕
...
#elif SCREEN_DRIVER == SCREEN_DRIVER_ST7789V

	printf("lcdInitST7789() driver: st7789v\n") ;
	...
	// 	spi_master_write_data_byte(dev, 0x70);
	// else
...
//--------------------------------ST7789V gamma setting---------------------------------------//
	...
	spi_master_write_data_byte(dev, 0x42); 
	...
	spi_master_write_data_byte(dev, 0x1e);
...
// y2:End Y coordinate
// colors:colors
void lcdDrawRectPixels(TFT_t * dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t * colors) {
...
uint16_t rgb565_conv_inv(uint16_t r,uint16_t g,uint16_t b) {
