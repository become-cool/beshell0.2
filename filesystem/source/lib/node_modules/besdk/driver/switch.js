...
class Switch extends EventEmitter {
    ...
    begin(pinInput, pinOutput) {
        ...
        if( (pinInput.gpio==34 || pinInput.gpio==36) && (pinOutput.gpio==35 || pinOutput.gpio==39)) {
            // 交换 input / output pin
            let tmp = pinInput
            pinInput = pinOutput
            pinOutput = tmp
        ...
        if( pinOutput=='vcc' || pinOutput.gpio==34 || pinOutput.gpio==36 ) {   // 34/36 被固定上拉了
            this._mode = 0
        }
        ...
        console.log("output",pinOutput.gpio)
        console.log("input",pinInput.gpio)
