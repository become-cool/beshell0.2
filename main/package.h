...
#define PKGLEN_WITHOUT_DATA         6
#define PKGLEN_MAX_DATA             255
#define BUFFLEN_RECEIVE             270     // > PKGLEN_MAX_DATA + PKGLEN_WITHOUT_DATA
...
#define CMD_RSPN	                3		// CMD_CALL 的返回
#define CMD_EXCEPTION	            4		// CMD_RUN/CMD_CALL 的异常
#define CMD_CALLBACK                5		// 执行本地js代码(远程发起)
#define CMD_OUTPUT	                6		// 远程输出
...
typedef void (*PkgCmdProcess)(uint8_t pkgId, uint8_t pkgCmd, uint8_t pkgDatLen, void * ctx);
...
uint8_t Pack(uint8_t * pkg, uint8_t pkgId, uint8_t cmd, uint8_t * data, uint8_t datalen) ;
